...
2. **Estimated from the pricing table.** A direct provider (Anthropic / OpenAI / Bedrock) reports only tokens. When you configure a `pricingStore` (`pricingStores.lucid()`), cost is estimated from the `agent_model_pricing` table (the current price rows are fetched once per run and folded into `usage.costUsd`):
   ...
   against the *current* pricing row for the model. With no `pricingStore` bound, or for a model with no price row, `cost_usd` stays **`null`** (never a fabricated `0` — a real $0 turn and "we don't know" stay distinguishable); the tokens still count toward quota and totals. Seed prices once with `seedModelPrices(store, [...])`.
