...
Inside the provider, the private `#assertOwner(ctx, actor, ownerRef, kind, governanceAuthorize)` helper wires this into the request: it looks up the record's owner (`service.runOwner(id)` / `service.threadOwner(id)`), computes `privileged` by running the same `governanceAuthorize` gate used for the governance read-model, then applies `evaluateOwnership`. On a non-`ok` verdict it replies the verdict's status (`404` → `"<kind> not found"`, `403` → `"forbidden"`) and short-circuits the handler.
...
Its type is `AgentGovernanceAuthorize = (actor, ctx) => boolean | Promise<boolean>`, and it is applied by the router-free `evaluateGovernanceGate` (also exported), which is **fail-closed**: an `authorize` that *throws* denies with `403` (and surfaces the message), exactly like a `false` return.

- **Omit it** and any resolved (authenticated) actor may read the governance read-model — the historical behavior, correct only when every authenticated caller is trusted staff. The provider logs a startup warning when the governance routes are mounted without a gate.
- The gate **does not** cover `GET /agent/approvals/mine`, which is always scoped to the calling actor's own pending approvals — so a non-admin surface can poll its own approvals even while the cross-actor read-model is ADMIN-only.
- The same predicate also feeds the object-level ownership check above: passing `governanceAuthorize` is what makes a caller `privileged` (may act across actors). It mirrors [`@adonis-agora/agent-dashboard`](/docs/agent/governance/dashboard)'s `authorize` hook, so the JSON routes and the console SPA that reads them can be gated with the *same* predicate.
