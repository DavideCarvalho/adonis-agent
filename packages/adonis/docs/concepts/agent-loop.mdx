...
At the center of the library is `runAgentLoop` — one provider-agnostic function that drives a single agent turn. It is the same body whether the turn runs in-process or (in the future) as a durable workflow. Understanding it is the fastest way to understand everything else.
...
| Hook | Inline runner | Durable runner (deferred) |
| --- | --- | --- |
...
<Callout type="info">
The inline runner is single-replica: pending approvals and the token sink live in process memory. Horizontal scaling and crash-survival are the job of the durable runner — see the [Roadmap](/docs/agent/roadmap).
</Callout>
