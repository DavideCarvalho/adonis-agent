...
Under the **inline runner**, delegation runs as a nested in-process loop on a transient sub-thread. Because a nested sub-agent has no human to prompt, its `action` tools are auto-declined rather than left hanging — so keep human-approval actions on the top-level agent. Under the future **durable runner**, the same delegation maps to a durable child workflow. See the [agent loop](/docs/agent/concepts/agent-loop) and the [Roadmap](/docs/agent/roadmap).
