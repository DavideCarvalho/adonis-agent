...
| `InMemoryGovernanceQueries` | `AgentGovernanceQueries` | The read-model (spend by model/actor, usage trend, recent tool calls/threads) over in-memory data, with a settable `InMemoryModelPrice` table. |
...
for await (const chunk of sink.subscribe(runId)) text += decoder.decode(chunk)
```
