 ...
 * 4. publishes `config/mcp.ts` (the MCP endpoint config; the MCP provider is separate);
 * 5. publishes the Lucid migration for the five agent tables (run `node ace migration:run`; delete it
 *    if you only use the in-memory store);
 * 6. publishes the additive run-tracking migration (`agent_run` table + `run_id` columns) — run it
